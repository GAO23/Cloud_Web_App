...
public class TimeSlot {
    ...
    public void decrementAllTASlotCount(){
        for(DayOfWeek key : tas.keySet()){
            for(TeachingAssistantPrototype ta : tas.get(key)){
                ta.decrementSlotDueToCombo();
            }
                 
        }
    }
    
    public void restorAllTASlotCount(){
        for(DayOfWeek key : tas.keySet()){
            for(TeachingAssistantPrototype ta : tas.get(key)){
                ta.restoreSlotCount();
            }
    ...
    public void printTAs(){
        System.out.println("Debugging(TA objects): " + this.tas);
        System.out.println("Debugging(Cell Text): " + this.dayText);
    }
